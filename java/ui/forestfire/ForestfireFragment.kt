...
class ForestfireFragment : Fragment(R.layout.fragment_forestfire), ForestfireAdapter.OnItemClickListener {
    ...
    // onClick() for værvarsel til hvert sted på Forestfire
    override fun onItemClickForestfire(data: ForestfireHolder) {
        viewModel.onClickForestfireData = data
        // Hindrer appen i å krasje dersom brukeren trykker to ganger raskt
        if (!viewModel.elementOnClick) {
            findNavController().navigate(R.id.action_ForestfireFragment_to_BottomSheetBottomSheetForestfireOnClick)
    ...
    private fun setForestfireAdapter(){
        if (mForefireAdapter == null) mForefireAdapter = ForestfireAdapter(this)
